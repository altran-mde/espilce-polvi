...
if [[ $profiles == *"publish"* ]]; then
	if curl --output /dev/null --silent --head --fail --user $ArtifactStoreUserName:$ArtifactStorePassword "https://nexus-p2.manatree.io/nexus/content/sites/MDEAssets/espilce-polvi/$updateSite/content.jar"; then
...
mvn clean install -P$profiles \
	-f releng/org.espilce.polvi.parent/pom.xml \
